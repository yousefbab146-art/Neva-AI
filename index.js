...
// Gemini API - direkt fetch ile v1 endpoint kullanıyoruz
...
// ============================================================
// GEMINI AI - Direkt v1 API
// ============================================================
const GEMINI_MODEL = 'gemini-1.5-flash';
const GEMINI_API_URL = `https://generativelanguage.googleapis.com/v1/models/${GEMINI_MODEL}:generateContent`;
const SYSTEM_INSTRUCTION = `Sen "Neva" adında, Neva Development Discord sunucusunun yapay zeka asistanısın.
...
Eğer bir görsel (ekran görüntüsü, fotoğraf vb.) gönderilirse onu da analiz et ve yorumla.`;

async function askGemini(parts) {
    const fetch = (await import('node-fetch')).default;
    const modifiedParts = [{ text: "SİSTEM TALİMATI: " + SYSTEM_INSTRUCTION + "\n\nKULLANICI MESAJI:\n" }, ...parts];
    const body = {
        contents: [{ role: 'user', parts: modifiedParts }]
    };
...
client.on('messageCreate', async message => {
    ...
    try {
        ...
        if (message.content) promptParts.push({ text: message.content });
        ...
        if (message.attachments.size > 0) {
            for (const [, attachment] of message.attachments) {
                if (attachment.contentType && attachment.contentType.startsWith('image/')) {
                    ...
                    promptParts.push({ inlineData: { mimeType: attachment.contentType, data: base64 } });
                    if (!message.content) promptParts.push({ text: 'Bu görseli analiz et ve detaylıca açıkla.' });
                }
            }
        }
        ...
        // Gemini'ye sor
        const responseText = await askGemini(promptParts);
        ...
        await sendLog(client, 'error', {
            user:  { id: message.author.id, tag: message.author.tag },
            extra: 'Gemini API Hatası: ' + err.message
        });
        await message.reply({
            embeds: [new EmbedBuilder()
                .setColor('#ED4245')
